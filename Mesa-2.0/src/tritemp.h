/* $Id: tritemp.h,v 1.6 1996/10/01 04:13:09 brianp Exp $ */
...

   {
	 ...
	 dbdy = oneOverArea * (eMaj.dx * eBot_db - eMaj_db * eBot.dx);
...
#if INTERP_STW
      {
         ...
         dtdy = oneOverArea * (eMaj.dx * eBot_dt - eMaj_dt * eBot.dx);
#if INTERP_UV
         eMaj_du = VB->TexCoord[vMax][2]*wMax - VB->TexCoord[vMin][1]*wMin;
         eBot_du = VB->TexCoord[vMid][2]*wMid - VB->TexCoord[vMin][1]*wMin;
         dudx = oneOverArea * (eMaj_du * eBot.dy - eMaj.dy * eBot_du);
         dudy = oneOverArea * (eMaj.dx * eBot_du - eMaj_du * eBot.dx);

         eMaj_dv = VB->TexCoord[vMax][3]*wMax - VB->TexCoord[vMin][3]*wMin;
         eBot_dv = VB->TexCoord[vMid][3]*wMid - VB->TexCoord[vMin][3]*wMin;
...
#endif
#if INTERP_RGB
               fr = (GLfixed)(VB->Color[vLower][0] + drdx * adjx + drdy * adjy) + FIXED_HALF;

               fdrOuter = SignedFloatToFixed(drdy + dxOuter * drdx);

               fg = (GLfixed)(VB->Color[vLower][1] + dgdx * adjx + dgdy * adjy) + FIXED_HALF;
               fdgOuter = SignedFloatToFixed(dgdy + dxOuter * dgdx);

               fb = (GLfixed)(VB->Color[vLower][2] + dbdx * adjx + dbdy * adjy) + FIXED_HALF;
               fdbOuter = SignedFloatToFixed(dbdy + dxOuter * dbdx);
#endif
#if INTERP_ALPHA
               fa = (GLfixed)(VB->Color[vLower][3] + dadx * adjx + dady * adjy) + FIXED_HALF;
               fdaOuter = SignedFloatToFixed(dady + dxOuter * dadx);
#endif
#if INTERP_INDEX
               fi = (GLfixed)(VB->Index[vLower] * FIXED_SCALE + didx * adjx + didy * adjy) + FIXED_HALF;
