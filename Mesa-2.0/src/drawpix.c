/* $Id: drawpix.c,v 1.3 1996/09/27 01:26:25 brianp Exp $ */
                                ...
                                const void *pixels )
{
   ...
   if (x<0 || y<0 || x+width>ctx->Buffer->Width || y+height>ctx->Buffer->Height) {
      return GL_FALSE;
   }
                    ...
                    GLenum format, GLenum type, const GLvoid *pixels )
{
   ...
   if (ctx->ExecuteFlag) {
      gl_drawpixels( ctx, width, height, format, type, image );
      if (!ctx->CompileFlag) {
